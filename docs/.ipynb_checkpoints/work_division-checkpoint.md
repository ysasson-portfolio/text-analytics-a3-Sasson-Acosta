...
## Team Members
- **Student A:** Yarden Sasson  
- **Student B:** Brandon Acosta  

## Responsibility Table

| Responsibility Area | Yarden (Student A) | Brandon (Student B) |
|---|---|---|
| **Model Ownership** | LDA topic modeling using CountVectorizer | NMF topic modeling using TF-IDF |
| **EDA / Preprocessing** | Led the original `01_eda.ipynb` and `02_preprocessing.ipynb` workflow, including dataset setup, cleaning, and tokenization | Assisted in running/debugging preprocessing across devices and making notebook path/output adjustments so the workflow could run on a second machine |
| **Feature Engineering** | Built CountVectorizer inputs and count matrices for LDA across the 3 groups | Built TF-IDF matrices for NMF across the 3 groups |
...
## Notes
- The project repository and notebook workflow were shared across both students.
- Topic interpretation and business insight development were completed collaboratively in accordance with the assignment requirements.